...
import com.wavesenterprise.utils.LcgRandom

class CftConsensus(override val ng: NG, override val blockchainSettings: BlockchainSettings, val cftSettings: CftSettings) extends PoALikeConsensus {
  ...
  def expectedValidators(parentBlockId: BlockId, timestamp: Long, currentMiner: Address): Either[GenericError, Set[Address]] = {
    val activeMinersWithoutCurrent = ng.miners.currentMinersSet(timestamp) - currentMiner
    ...
    if (activeMinersWithoutCurrent.size > maxValidatorsCount) {
      ...
      ng.lastBlockIds(parentBlockId, lastBlockCountForSeed)
        .map { lastBlockIds =>
    ...
    super.microBlockConsensusValidation(microBlock) >> {
      microBlock match {
        ...
        case voteMicro: VoteMicroBlock =>
          for {
            lastBlock <- ng.lastBlock.toRight(GenericError(s"Last block not found"))
