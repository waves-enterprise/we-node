...
import com.wavesenterprise.transaction.transfer.{TransferDescriptor, _}
...
class MassTransferTransactionV1Specification extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers with TransactionGen {
  ...
  property(testName = "JSON format validation") {
    forAll(accountGen) { senderAccount =>
      ...
      val tx = MassTransferTransactionV1
        .create(
