...
class PrivacyServiceImplSpec
  ...
  private val apiKey                 = "we"
  private lazy val apiKeyHash        = Base58.encode(crypto.secureHash(apiKey))
  private lazy val authSetting       = AuthorizationSettings.ApiKey(apiKeyHash, apiKeyHash)
  ...
  "#sendLargeData" - {
    ...
    val service          = new PrivacyServiceImpl(mockedApiService, true, authSetting, privacyServiceSettings, ownerAddress, time, NodeMode.Default)
