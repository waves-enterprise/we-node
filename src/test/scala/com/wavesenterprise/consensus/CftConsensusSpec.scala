...
import com.wavesenterprise.state.{NG, _}
...
class CftConsensusSpec extends FreeSpec with Matchers with ScalaCheckPropertyChecks with MockFactory with TransactionGen {
    ...
    "are same for different runs" in {
      forAll(minerQueueWithActiveTsGen(), Gen.nonEmptyListOf(blockIdGen), blockIdGen) {
        case ((minerQueue, activeTs), lastBlockIds, parentBlockId) =>
          val ng = mock[NG]
          (ng.miners _).expects().returns(minerQueue).twice()
          (ng.lastBlockIds(_: BlockId, _: Int)).expects(parentBlockId, *).returns(Some(lastBlockIds)).twice()
          ...
          val consensus = new CftConsensus(ng, TestBlockchainSettings.Default, settings)
    ...
    "are different for different last block ids" in {
      forAll(minerQueueWithActiveTsGen(minMinerCount = 30, maxMinerCount = 100), Gen.nonEmptyListOf(blockIdGen), blockIdGen, blockIdGen) {
        case ((minerQueue, activeTs), lastBlockIds, parentBlockId, additionalBlockId) =>
          val ng = mock[NG]
          (ng.miners _).expects().returns(minerQueue).twice()
          (ng.lastBlockIds(_: BlockId, _: Int)).expects(parentBlockId, *).returns(Some(lastBlockIds)).once()
          (ng.lastBlockIds(_: BlockId, _: Int)).expects(parentBlockId, *).returns(Some(additionalBlockId +: lastBlockIds)).once()
    ...
    "are all active miners if their number does not exceed maxValidators" in {
      forAll(minerQueueWithActiveTsGen(), blockIdGen) {
        case ((minerQueue, activeTs), parentBlockId) =>
          val ng = mock[NG]
          (ng.miners _).expects().returns(minerQueue).once()
          (ng.lastBlockIds(_: BlockId, _: Int)).expects(*, *).never()
    ...
    "fails if parent block has no votes" in {
      forAll(timestampGen, blockIdGen, blockIdGen, accountGen, Gen.nonEmptyListOf(randomTransactionGen)) {
        case (ts, parentId, parentReferenceId, blockSigner, txs) =>
          ...
          val ng = mock[NG]
          (ng.blockHeaderByIdWithLiquidVariations(_: ByteStr)).expects(parentId).returns(Some(parentBlock.blockHeader)).once()

          val consensus = new CftConsensus(ng, TestBlockchainSettings.Default, DefaultCftSettings)
    ...
    "fails if block has not enough votes" in {
      ...
      forAll(preconditions) {
        case (parentId, parentReferenceId, parentVersion, blockSigner, voteSigner, txs, minerQueue, ts, lastBlockIds) =>
          ...
          val ng = mock[NG]
          (ng.miners _).expects().returns(minerQueue).once()
          (ng.lastBlockIds(_: BlockId, _: Int)).expects(parentId, *).returns(Some(lastBlockIds)).once()
          (ng.blockHeaderByIdWithLiquidVariations(_: ByteStr)).expects(parentId).returns(Some(parentBlock.blockHeader)).once()

          val cftSettings = DefaultCftSettings.copy(maxValidators = PositiveInt(unsatisfactoryValidatorsCount))
          val consensus   = new CftConsensus(ng, TestBlockchainSettings.Default, cftSettings)
