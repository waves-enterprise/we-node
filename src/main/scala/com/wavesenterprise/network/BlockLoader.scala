...
class BlockLoader(
    ...
    incomingBlockEvents: ChannelObservable[Block],
  ...
  private val broadcastBlocksQueue = ConcurrentSubject.publish[(Channel, Block)](OverflowStrategy.DropOld(10))

  private val extensionsQueue = AsyncVar.empty[(Channel, ExtensionBlocks)]()

  def broadcastBlockEvents: ChannelObservable[Block] = {
    val eventEqByBlockId: Eq[(Channel, Block)] = {
      case ((_, block1), (_, block2)) => block1.uniqueId == block2.uniqueId
    }
  ...
  private def onBlock(state: State, channel: Channel, block: Block): Task[State] = Task.defer {
    state.loaderState match {
      case ExpectingBlocks(ch, requested, expected, received, rest, timeoutHandler) if ch == channel && expected.contains(block.uniqueId) =>
        val isLastExpected = Set(block.uniqueId) == expected

        if (isLastExpected) {
          timeoutHandler.cancel()

          val blockById              = (received + block).map(b => b.uniqueId -> b).toMap
          ...
          val extensionSignatures = requestedOrderedBlocks.map(_.uniqueId)
          ...
          Task(state.withLoaderState(ExpectingBlocks(ch, requested, expected - block.uniqueId, received + block, rest, timeoutHandler)))
        }
      case _: ExpectingBlocks =>
        log.debug(s"Received unexpected block '${block.uniqueId}' from '${id(channel)}', ignoring it")
        Task.pure(state)
      case _ =>
        log.trace(s"Received broadcast block '${block.uniqueId}' from '${id(channel)}'")
        Task.fromFuture(broadcastBlocksQueue.onNext(channel -> block)).as(state)
    }
  }
  ...
  private def requestBlockBatch(channel: Channel, state: State, allSignatures: Seq[BlockId]): State = {
    ...
    if (channel.hasAttr(Attributes.MultipleExtBlocksAttribute)) {
      channel.writeAndFlush(GetBlocks(batch))
    } else {
      batch.foreach(sign => channel.write(GetBlock(sign)))
      channel.flush()
    }
  ...
  private case class BlockReceived(channel: Channel, block: Block)                      extends StateUpdateReason
