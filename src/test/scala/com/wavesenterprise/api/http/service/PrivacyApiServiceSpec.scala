...
import com.wavesenterprise.api.http.{PoliciesMetaInfoRequest, PoliciesMetaInfoResponse, PolicyIdWithDataHash, PrivacyDataInfo, SendDataRequest}
...
import com.wavesenterprise.transaction.PolicyDataHashTransactionV1
...
class PrivacyApiServiceSpec extends FunSpecLike with Matchers with MockFactory with TransactionGen with ScalaCheckPropertyChecks with AsyncTest {
  ...
  val recipientAddress: Set[Address] = severalAddressGenerator().generateSample().toSet
  ...
  it("data is hashed by sha256") {
    ...
    forAll(byteArrayGen(14000)) { originDataBytes =>
      ...
      val request =
        SendDataRequest(1, sender.address, policyIdByteStr.toString, Some(dataString), dataHashString, privacyDataDataInfo, fee).toPolicyItem
  ...
  it("validate policy data size") {
    ...
    val request =
      SendDataRequest(1, sender.address, policyIdByteStr.toString, Some(dataString), "do not care", privacyDataDataInfo, fee).toPolicyItem
  ...
  it("validate feeAssetId with version data size") {
    ...
    val request =
      SendDataRequest(1, sender.address, policyIdByteStr.toString, Some(dataString), "do not care", privacyDataDataInfo, fee, feeAssetId).toPolicyItem
  ...
  it("validate policy data fileName length") {
    ...
    val request =
      SendDataRequest(1, sender.address, policyIdByteStr.toString, Some("data"), "do not care", privacyDataDataInfo, fee).toPolicyItem
