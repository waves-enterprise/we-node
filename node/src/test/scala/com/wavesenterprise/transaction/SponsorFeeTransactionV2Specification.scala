...
class SponsorFeeTransactionV2Specification
  ...
  val NgAndSponsorshipSettings: FunctionalitySettings = TestFunctionalitySettings.Enabled
    .copy(
      preActivatedFeatures = Map(
        ...
        OtherTxTypesAtomicSupport.id -> 0,
      ),
  ...
  property("miner receives one satoshi less than sponsor pays") {
    val setup = for {
      ...
      fee <- smallFeeGen
      sponsor =
        SponsorFeeTransactionV2.selfSigned(acc, issue.id(), isEnabled = true, fee = One, timestamp = ts, atomicBadge = None).explicitGet()
  ...
  property("miner receives one satoshi more than sponsor pays") {
    val setup = for {
      ...
      issue      = IssueTransactionV2.selfSigned(currentChainId, acc, name, desc, quantity, decimals, reissuable, fee, ts, None).explicitGet()
      assetRatio = 1
      sponsor    = SponsorFeeTransactionV2.selfSigned(acc, issue.id(), isEnabled = true, One, ts, atomicBadge = None).explicitGet()
      fee <- smallFeeGen
  ...
  property("sponsorship changes in the middle of a block") {
    val setup = for {
      ...
      assetRatio = 1
      fee <- smallFeeGen
      sponsor1 = SponsorFeeTransactionV2.selfSigned(acc, issue.id(), isEnabled = true, One, ts, atomicBadge = None).explicitGet()
      transfer1 = TransferTransactionV2
        .selfSigned(acc, None, Some(issue.id()), ts + 1, 1, fee = fee * assetRatio, acc.toAddress, Array())
        .explicitGet()
      assetRatio2 = 1
      sponsor2    = SponsorFeeTransactionV2.selfSigned(acc, issue.id(), isEnabled = true, One, ts + 2, atomicBadge = None).explicitGet()
      transfer2 = TransferTransactionV2
