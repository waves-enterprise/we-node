...
import com.wavesenterprise.database.{Key, Keys, PrivacyState, readTxIds}
...
import com.wavesenterprise.state.{ByteStr, MiningConstraintsHolder, NG}
...
abstract class StateMigrationBase extends App with ScorexLogging {
      ...
      case Success(files) if files.findAny().isPresent =>
        Try {
          ...
          case Success(_) =>
            ...
            val migrationService = buildMigrationService(lazySourceAsTargetDb)
  ...
  protected def buildMigrationService(lazySourceAsTargetDb: Coeval[RocksDBStorage]) =
...
object StateMigration extends StateMigrationBase
...
class MigrationService(
  ...
  protected val (persistentStorage, targetState) = blockchainFactory(weSettings, targetDb, time, schedulers)

  protected def blockchainFactory(settings: WESettings,
  ...
  private val consensus = Consensus(weSettings.blockchain, targetState, time)
  ...
  def runMigration(bufferSize: Int, backpressure: Int): CancelableFuture[Unit] = {
    val certs = loadCerts(weSettings, persistentStorage)
    checkGenesis(weSettings, targetState, certs)
  ...
  private def processBlock(tuple: (Int, Block)): Unit = {
    ...
    blocking {
      consensus
        ...
        .flatMap { postAction =>
          targetState.processBlock(block, postAction, Hard, isOwn = true, certChainStore = CertChainStore.empty) // TODO: Another task?
        }
