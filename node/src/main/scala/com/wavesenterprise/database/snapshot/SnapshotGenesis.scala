...
import com.wavesenterprise.state.diffs.BlockDifferBase.BlockDiffResult
...
class SnapshotGenesis(snapshotOpener: SnapshotOpener, weSettings: WESettings, miner: PrivateKeyAccount, time: Time) extends ScorexLogging {
  ...
  protected val senderRoleEnabled: Boolean = weSettings.blockchain.custom.genesis.senderRoleEnabled
  ...
  def append(genesis: Block): Task[Either[ValidationError, Unit]] = {
    snapshotOpener.withSnapshot { snapshot =>
      Task {
        for {
          blockDiff <- buildBlockDiff(genesis, snapshot)
          (diff, fees, _) = blockDiff
        } yield {
          snapshot.append(diff, fees, genesis, certificates = diff.certByDnHash.values.toSet)
        }
      }
    }
  }

  protected def buildBlockDiff(genesis: Block, snapshot: RocksDBWriter): Either[ValidationError, BlockDiffResult] =
    BlockDiffer.fromBlock(
      blockchainSettings = weSettings.blockchain,
      snapshotSettings = weSettings.consensualSnapshot,
      blockchain = snapshot,
      permissionValidator = PermissionValidator(senderRoleEnabled),
      maybePrevBlock = None,
      block = genesis,
      constraint = MiningConstraint.Unlimited,
      txExpireTimeout = MaxTimePrevBlockOverTransactionDiff,
