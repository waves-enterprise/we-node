...
object PrivacyApiService {
  ...
  case class ValidSendDataSetup(policyId: ByteStr,
...
class PrivacyApiService(val state: Blockchain with PrivacyLostItemUpdater,
  ...
  private def internalSendData(policyItem: PolicyItem,
    ...
    import setup.{policyId, signer, dataHash, feeAssetId, atomicBadge}
    ...
    def checkDiffIfNeed(tx: Transaction): EitherT[Task, ApiError, Diff] = {
      if (broadcast)
        EitherT.fromEither[Task](txBroadcaster.txDiffer(tx)).leftMap(validationToApiError)
      else
        EitherT.rightT[Task, ApiError](Diff.empty)
    }

    def broadcastTxIfNeed(tx: PolicyDataHashTransaction, diff: Diff): EitherT[Task, ApiError, Unit] =
      if (broadcast) {
        EitherT.right[ApiError] {
          Task {
            val txWithSize = TransactionWithSize(tx.bytes().length, tx)
            txBroadcaster.forceBroadcast(txWithSize, diff)
    ...
    (for {
      ...
      diff          <- checkDiffIfNeed(tx)
      _             <- saveData(hashInStorage)
      _             <- broadcastTxIfNeed(tx, diff)
  ...
  private def validatePolicyItem(policyItem: PolicyItem): Either[ValidationError, ValidSendDataSetup] =
    PrivacyMetrics.measureEither(SendDataRequestValidation, policyItem.policyId, policyItem.hash) {
      ...
      } yield ValidSendDataSetup(policyId, privateKey, dataHash, feeAssetId, policyItem.atomicBadge)
    }
    ...
    PrivacyMetrics.measureEither(SendDataRequestValidation, policyItem.policyId, policyItem.hash) {
      for {
        ...
        sender <- Address
          ...
          .leftMap(err => GenericError(s"Failed to decode sender address '${policyItem.sender}', reason: $err"))
        privateKey <- sendersPrivateKey(sender, policyItem.password)
      } yield ValidSendDataSetup(policyId, privateKey, pdh, feeAssetId, policyItem.atomicBadge)
    }
