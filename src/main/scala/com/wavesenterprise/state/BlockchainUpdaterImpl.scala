...
import com.wavesenterprise.state.diffs.BlockDiffer.DiffResult
...
class BlockchainUpdaterImpl(state: Blockchain with PrivacyState, settings: WESettings, time: Time, schedulers: Schedulers)
  ...
  override def processBlock(block: Block,
                            ...
                            blockType: BlockType,
                            isOwn: Boolean,
                            alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, Option[DiscardedTransactions]] =
    writeLock {
      ...
      checkFeaturesImplementation >>
        innerNgState
          .map(ng => processBlockWithNg(block, ng, isOwn, alreadyVerifiedTxIds))
          .getOrElse(processBlockWithoutNg(block, isOwn, alreadyVerifiedTxIds))
  ...
  private def processBlockWithNg(block: Block,
                                 ng: NgState,
                                 isOwn: Boolean,
                                 alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, Option[(DiffResult, DiscardedTransactions)]] = {
    if (ng.base.reference == block.reference) {
      processBlockWithSameParent(block, ng, isOwn, alreadyVerifiedTxIds)
    } else {
      processBlock(block, ng, isOwn, alreadyVerifiedTxIds).map(Some(_))
    }
  }
  ...
  private def processBlock(block: Block,
                           ng: NgState,
                           isOwn: Boolean,
                           alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, (DiffResult, DiscardedTransactions)] = {
        ...
        if (isOwn || referencedForgedBlock.signatureValid()) {
          ...
          val diff = BlockDiffer
            .fromBlock(
              ...
              CompositeBlockchain.composite(state, referencedLiquidDiff, carry, new ContractValidatorsProvider(this)),
              permissionValidator,
              Some(referencedForgedBlock),
              block,
              constraint,
              settings.utx.txExpireTimeout,
              isOwn,
              alreadyVerifiedTxIds
            )

          diff.map { hardenedDiff =>
            state.append(referencedLiquidDiff, carry, referencedForgedBlock, ng.consensusPostActionDiff)
  ...
  private def processBlockWithSameParent(block: Block,
                                         ng: NgState,
                                         isOwn: Boolean,
                                         alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, Option[(DiffResult, DiscardedTransactions)]] = {
  ...
  private def processBlockWithoutNg(block: Block,
                                    isOwn: Boolean,
                                    alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, Some[(DiffResult, DiscardedTransactions)]] = {
  ...
  override def processMicroBlock(microBlock: MicroBlock, isOwn: Boolean, alreadyVerifiedTxIds: Set[ByteStr]): Either[ValidationError, Unit] =
    writeLock {
      innerNgState match {
        case None =>
          ...
          ng.lastMicroBlock match {
            ...
            case _ =>
              for {
                ...
                ng.append(microBlock, diff, carry, 0L)
  ...
  override def append(diff: Diff, carry: Long, block: Block, consensusPostActionDiff: ConsensusPostActionDiff): Unit = writeLock {
    state.append(diff, carry, block, consensusPostActionDiff)
  }
