...
import com.wavesenterprise.network.message.MessageSpec.{BlockSpec, MicroBlockResponseSpec, TransactionSpec}
...
object NodeAttributes {
  implicit val show: Show[NodeAttributes] = Show.show { attributes =>
    s"Node attributes [mode: ${attributes.nodeMode}] from address '${attributes.sender.toAddress}'"
  }
  ...
  def fromBytes(bytes: Array[Byte], sender: PublicKeyAccount): Either[String, NodeAttributes] = {
    ...
    } yield NodeAttributes(nodeMode, p2pTlsEnabled = false, sender)
  }
}
