...
object SnapshotStarterApp extends SnapshotStarterAppBase

class SnapshotStarterAppBase extends TaskApp with ScorexLogging {
  ...
  protected def processSnapshot(dataDirectory: String,
  ...
  class NetworkClient(ownerKey: PrivateKeyAccount, genesisSignature: ByteStr, val networkSettings: NetworkSettings)
        ...
        .handler {
          new ChannelInitializer[SocketChannel] {
            override def initChannel(channel: SocketChannel): Unit = {
              val handlers = buildChannelHandlers(responsePromise, channel)
      ...
      channelFuture
        .addListener { future: io.netty.util.concurrent.Future[_] =>
          if (future.isSuccess) {
            ...
            connectPromise.success(channel)
          } else {
    ...
    protected def buildChannelHandlers(responsePromise: Promise[SnapshotNotification], channel: SocketChannel): IndexedSeq[ChannelHandler] = {
      IndexedSeq(
