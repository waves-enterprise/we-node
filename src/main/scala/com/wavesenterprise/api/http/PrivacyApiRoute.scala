...
class PrivacyApiRoute(val privacyService: PrivacyApiService,
  ...
  override val route: Route = pathPrefix("privacy") {
  ...
  def sendData: Route =
    withSizeLimit(PrivacyApiRoute.sendDataEntityMaxSize) {
      (pathPrefix("sendData") & post & parameter("broadcast".as[Boolean] ? true) & privacyUserAuth & nonWatcherFilter & withPrivacyEnabled) {
        broadcast =>
          json[JsObject] { jsv =>
            sendPolicyItem(jsvToSendDataRequest(jsv, broadcast).map(_.toPolicyItem), broadcast)
          }
      }
    }
  ...
  def sendDataV2: Route =
    withSizeLimit(PrivacyApiRoute.sendDataEntityMaxSize) {
      (pathPrefix("sendDataV2") & post & parameter("broadcast".as[Boolean] ? true) & privacyUserAuth & nonWatcherFilter & withPrivacyEnabled) {
        broadcast =>
  ...
  def sendLargeData: Route = withSizeLimit(PrivacyApiRoute.sendLargeDataEntityMaxSize) {
    (pathPrefix("sendLargeData") & post & parameter("broadcast".as[Boolean] ? true) & privacyUserAuth & withPrivacyEnabled & nonWatcherFilter & withLargeObjectFeatureActivated) {
      toBroadcast =>
        entity(as[Multipart.FormData]) { formData =>
                  ...
                  bodyPart.toStrict(serviceSettings.metaDataAccumulationTimeout).flatMap { bp =>
                    handleApiError {
                      for {
                        ...
                        policyItem = sendDataRequest.toPolicyItem
                        setup <- privacyService.validateLargePolicyItem(sendDataRequest.toPolicyItem).leftMap(ApiError.fromValidationError)
                      } yield {
