...
class MassTransferTransactionV3Specification extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers with TransactionGen {
  ...
  property(testName = "JSON format validation") {
    forAll(accountGen) { senderAccount =>
      ...
      val tx = MassTransferTransactionV3
        .create(
