...
import com.wavesenterprise.matcher.ValidationMatcher
import com.wavesenterprise.state.ByteStr
import com.wavesenterprise.state.diffs._
import com.wavesenterprise.transaction.assets.exchange.{AssetPair, Order, OrderType, _}
...
class OrderSpecification extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers with TransactionGen with ValidationMatcher with NTPTime {
  ...
  property("Order transaction serialization roundtrip") {
    ...
    forAll(orderV2Gen) { order =>
      ...
      recovered.bytes() shouldEqual order.bytes()
      recovered.idStr() shouldBe order.idStr()
      recovered.senderPublicKey.publicKey.getEncoded shouldBe order.senderPublicKey.publicKey.getEncoded
      recovered.matcherPublicKey shouldBe order.matcherPublicKey
      recovered.assetPair shouldBe order.assetPair
      recovered.orderType shouldBe order.orderType
      recovered.price shouldBe order.price
