...
/** The `CircuitBreaker` is used to provide stability and prevent
  ...
  */
/**
  * Difference between this implementation and monix [[monix.catnap.CircuitBreaker]] is ability to catch some exceptions
  * which will not change CircuitBreaker state and just throw exception outside ([[protectWithExceptions()]]
 **/
final class CircuitBreakerWithExceptions[F[_]] private (_stateRef: AtomicAny[CircuitBreakerWithExceptions.State],
                                                        _maxFailures: Int,
  ...
  import CircuitBreakerWithExceptions._
  ...
  /** Returns the current [[CircuitBreaker.State]], meant for
    * debugging purposes.
    */
  val state: F[CircuitBreakerWithExceptions.State] =
  ...
  def protectWithExceptions[A](task: F[A], checkFatalExceptions: ExceptionsMatcher): F[A] = {
    def innerUnsafeProtect(task: F[A]): F[A] = {
      stateRef.get match {
        case Closed(_) =>
          val bind = maybeMarkOrResetFailures.asInstanceOf[Either[Throwable, A] => F[A]]
          task.attempt.flatMap {
            case right @ Right(_) => bind(right)
            case left @ Left(exception) =>
              if (checkFatalExceptions(exception)) {
                F.raiseError[A](exception)
              } else {
                bind(left)
              }
          }
    ...
    F.suspend(innerUnsafeProtect(task))
  ...
  /** Returns a new circuit breaker that wraps the state of the source
    ...
    */
  def doOnRejectedTask(callback: F[Unit]): CircuitBreakerWithExceptions[F] = {
    val onRejected = this.onRejected.flatMap(_ => callback)
    new CircuitBreakerWithExceptions(
      _stateRef = stateRef,
  ...
  /** Returns a new circuit breaker that wraps the state of the source
    ...
    * @return a new circuit breaker wrapping the state of the source
    */
  def doOnClosed(callback: F[Unit]): CircuitBreakerWithExceptions[F] = {
  ...
  /** Returns a new circuit breaker that wraps the state of the source
    ...
    * @return a new circuit breaker wrapping the state of the source
    */
  def doOnHalfOpen(callback: F[Unit]): CircuitBreakerWithExceptions[F] = {
  ...
  /** Returns a new circuit breaker that wraps the state of the source
    ...
    * @return a new circuit breaker wrapping the state of the source
    */
  def doOnOpen(callback: F[Unit]): CircuitBreakerWithExceptions[F] = {
...
/**
  ...
  */
object CircuitBreakerWithExceptions {
  ...
  def unsafe[F[_]](
      ...
      maxResetTimeout: Duration = Duration.Inf,
      padding: PaddingStrategy = NoPadding
  )(implicit F: Sync[F], clock: Clock[F]): CircuitBreakerWithExceptions[F] = {

    CircuitBreakerWithExceptions[F].unsafe(
      maxFailures = maxFailures,
      resetTimeout = resetTimeout,
      exponentialBackoffFactor = exponentialBackoffFactor,
      maxResetTimeout = maxResetTimeout,
  ...
  /**
    ...
    *
    */
  final class Builders[F[_]](val F: Sync[F]) extends AnyVal {
    ...
    def of(
        ...
        padding: PaddingStrategy = NoPadding
    )(implicit clock: Clock[F]): F[CircuitBreakerWithExceptions[F]] = {
    ...
    def unsafe(
        ...
        onOpen: F[Unit] = F.unit,
        padding: PaddingStrategy = NoPadding
    )(implicit clock: Clock[F]): CircuitBreakerWithExceptions[F] = {

      val atomic = Atomic.withPadding(Closed(0): State, padding)
      new CircuitBreakerWithExceptions[F](
        _stateRef = atomic,
        _maxFailures = maxFailures,
        _resetTimeout = resetTimeout,
        _exponentialBackoffFactor = exponentialBackoffFactor,
  ...
  /** [[State]] of the [[CircuitBreaker]] in which the circuit
    ...
    *        completion.
    */
  final class Open private (val startedAt: Timestamp, val resetTimeout: FiniteDuration, private[docker] val awaitClose: CancelablePromise[Unit])
      extends State {
  ...
  object Open {
    ...
    /** Private builder. */
    private[docker] def apply(startedAt: Timestamp, resetTimeout: FiniteDuration, awaitClose: CancelablePromise[Unit]): Open =
  ...
  /** [[State]] of the [[CircuitBreaker]] in which the circuit
    ...
    *        completion.
    */
  final class HalfOpen private (val resetTimeout: FiniteDuration, private[docker] val awaitClose: CancelablePromise[Unit]) extends State {
  ...
  object HalfOpen {
    ...
    /** Private builder. */
    private[docker] def apply(resetTimeout: FiniteDuration, awaitClose: CancelablePromise[Unit]): HalfOpen =
