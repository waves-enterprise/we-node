...
/**
  * On every established connection schedules checks to determine if connection idles no more then [[IdleConnectionDetector.timeout]]
  * */
@Sharable
class IdleConnectionDetector(timeout: FiniteDuration) extends ChannelInboundHandlerAdapter with ScorexLogging {
