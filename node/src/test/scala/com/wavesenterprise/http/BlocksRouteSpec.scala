...
class BlocksRouteSpec
  ...
  routePath("{path with positive int path components}") - {
    "return Bad Request on not positive integers" in {
      ...
      val genStr: Gen[String]      = Gen.alphaNumStr suchThat (s => s.nonEmpty && Try(s.toInt).isFailure)
      ...
      val gen = for {
        ...
        (pathFunc(args), args.take(countArgs))
      }
      ...
      forAll(gen) {
        case (p, notValidPathComponents) =>
          Get(routePath(p)) ~> route ~> check {
            ...
            notValidPathComponents.foreach { pc =>
              error should include(s"'$pc'")
            }
          }
      }
