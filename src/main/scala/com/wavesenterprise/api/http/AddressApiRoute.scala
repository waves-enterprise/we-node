...
class AddressApiRoute(addressApiService: AddressApiService,
  ...
  /**
    ...
    **/
  def effectiveBalanceWithConfirmations: Route = (get & path("effectiveBalance" / Segment / IntNumber) & userAuth) {
  ...
  /**
    ...
    */
  def generatingBalanceAtHeight: Route = (get & path("generatingBalance" / Segment / "at" / IntNumber) & userAuth) {
    case (address, requestedHeight) =>
      withExecutionContext(scheduler) {
        complete {
  ...
  def getData: Route = (get & path("data" / Segment) & contractOrUserAuth) { address =>
    withExecutionContext(scheduler) {
      parameters('offset.as[Int].?, 'limit.as[Int].?) { (offset, limit) =>
        complete(addressApiService.accountData(address, offset, limit))
      }
    }
  }
  ...
  /**
    ...
    * Get wallet accounts addresses
    **/
  def seq: Route = (get & path("seq" / IntNumber / IntNumber) & userAuth) {
    case (start, end) if end - start > MaxAddressesPerRequest =>
      complete(TooBigArrayAllocation)
    case (start, end) if start < 0 || end < start =>
