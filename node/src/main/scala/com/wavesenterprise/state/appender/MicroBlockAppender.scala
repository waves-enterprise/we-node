...
class MicroBlockAppender(
  ...
  private def processLoadedMicroBlock(entry: ReceivedMicroBlock): Task[Unit] = {
    ...
    (for {
      ...
      _                 <- EitherT(appendMicroBlock(validMicroBlock, alreadyVerifiedTxIds = preValidatedTxIds, certChainStore = entry.certChainStore))
    } yield {
  ...
  def appendMicroBlock(
    ...
    def measuredAction: Either[ValidationError, Unit] = {
      blockchainUpdater.processMicroBlock(microBlock, isOwn, alreadyVerifiedTxIds, certChainStore).map { _ =>
        microBlock match {
