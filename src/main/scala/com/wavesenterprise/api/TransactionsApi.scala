...
trait TransactionsApi {
  ...
  def validateAndBroadcastTransaction(tx: Transaction): EitherT[Task, ApiError, Transaction] = {
    for {
      ...
      _ <- EitherT.fromEither[Task](txBroadcaster.broadcastIfNew(tx)).leftMap(ApiError.fromValidationError)
    } yield tx
  }
