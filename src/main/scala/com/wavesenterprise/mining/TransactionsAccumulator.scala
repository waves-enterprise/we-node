...
class TransactionsAccumulator(ng: NG,
  ...
  private[this] val txDiffer: TransactionDiffer = TransactionDiffer(
  ...
  def process(tx: Transaction): Either[ValidationError, Diff] = writeLock {
    if (processingAtomic) {
  ...
  private def processTransaction(tx: Transaction, atomically: Boolean = false): Either[ValidationError, Diff] = {
    ...
    if (conflictFound) {
      ...
      txDiffer(state, tx, atomically).map { txDiff =>
  ...
  def processAtomically(tx: Transaction): Either[ValidationError, Diff] = writeLock {
    if (processingAtomic) {
      processTransaction(tx, atomically = true)
    } else {
      Left(GenericError("Atomic transaction mining is not started"))
    }
  }

  def commitAtomic(tx: AtomicTransaction): Either[ValidationError, Diff] = writeLock {
    for {
      ...
      atomicTxDiff <- processTransaction(tx)
    } yield {
