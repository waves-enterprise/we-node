...
class SignedRequestsAtomicTest extends AnyFunSuite with Matchers with TransactionGen {
  ...
  test("AssetIssueRequest json parsing works") {
    val json =
      ...
      """
    val req = Json.parse(json).validate[SignedIssueV3Request].get
    ...
    req.atomicBadge shouldBe atomicBadge
    ...
    tx.atomicBadge shouldBe atomicBadge
  ...
  test("AssetReissueRequest json parsing works") {
    val json =
      s"""
        ...
        |}
      """.stripMargin
    val req = Json.parse(json).validate[SignedReissueV3Request].get
  ...
  test("AssetTransfer json parsing works") {
    val json =
      s"""
        ...
        |}
      """.stripMargin
    val req = Json.parse(json).validate[SignedTransferV3Request].get
  ...
  test("AssetBurnRequest json parsing works") {
    val json =
      s"""
        ...
        |}
      """.stripMargin
    val req = Json.parse(json).validate[SignedBurnV3Request].get
    req.senderPublicKey shouldBe "D6HmGZqpXCyAqpz8mCAfWijYDWsPKncKe5v3jq1nTpf5"
  ...
  "isEnabled": false
    ...
    val req = Json.parse(js1).validate[SignedSponsorFeeRequestV2].get.toTx.right.get
    ...
    for (js <- Seq(js2, js3, js4)) {
      val req = Json.parse(js).validate[SignedSponsorFeeRequestV2].get.toTx.right.get
