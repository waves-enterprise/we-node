...
object CompositeHttpService {
  def apply(routes: Seq[ApiRoute], settings: ApiSettings, httpRequestsCacheSettings: HttpRequestsCacheSettings): CompositeHttpService =
    new CompositeHttpService(routes, settings, httpRequestsCacheSettings)
}
