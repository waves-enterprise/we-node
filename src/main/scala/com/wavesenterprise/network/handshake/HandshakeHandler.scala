...
abstract class HandshakeHandler(blockchain: Blockchain,
  ...
  private def checkHandshake(signedHandshake: SignedHandshake): Validation[Unit] = {
    ...
    checkPubKey(signedHandshake, pubKeyFromBlockchain).leftFlatMap { _ =>
  ...
  private def checkHandshakeInfo(remoteHandshake: SignedHandshake): Either[String, Unit] = {
    ...
    } else if (remoteHandshake.chainId != remoteHandshake.chainId) {
  ...
  protected def sendLocalHandshake(ctx: ChannelHandlerContext): Unit = {
    val signedHandshake = SignedHandshakeV3.createAndSign(
      chainId = applicationInstanceInfo.chainId,
      nodeVersion = applicationInstanceInfo.nodeVersion,
      consensusType = applicationInstanceInfo.consensusType,
      declaredAddress = applicationInstanceInfo.declaredAddress,
      nodeNonce = applicationInstanceInfo.nodeNonce,
      nodeName = applicationInstanceInfo.nodeName,
      sessionPubKey = PublicKeyAccount(sessionKey.publicKey),
      ownerKey = ownerKey
    )
...
object HandshakeHandler extends ScorexLogging {
  ...
  def versionIsSupported(remoteVersion: NodeVersion): Boolean =
    remoteVersion >= NodeVersion(1, 0, 0)
  ...
  class Server(blockchain: Blockchain,
               ...
               initialParticipantsDiscoverResult: InitialParticipantsDiscoverResult,
               connectionAcceptor: PeerConnectionAcceptor)
      extends HandshakeHandler(blockchain, applicationInstanceInfo, ownerKey, connectPromise, initialParticipantsDiscoverResult, connectionAcceptor) {
