...
class PolicyDataHashV2Specification extends AnyFunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {
  ...
  ignore("wrong proofs") {
    forAll(createPolicyTransactionV2GenWithRecipients()) { createPolicyTxWithRecipientPrivKeys =>
      ...
      val PolicyDataWithTxV2(_, policyDataHashTx) = policyDataHashTransactionV2Gen(Gen.const(createPolicyTx.id.value.arr)).sample.get
      ...
      assertDiffEither(preconditions, blockWithPolicyDataHash, TestFunctionalitySettings.Enabled) { totalDiffEi =>
        inside(totalDiffEi) {
          case Right(diff) =>
            ...
            dataHashes should contain theSameElementsAs Set(policyDataHashTx.dataHash)
        }
      }
