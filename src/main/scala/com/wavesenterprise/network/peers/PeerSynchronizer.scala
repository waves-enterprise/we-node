...
import com.wavesenterprise.network.{
  Attributes,
  ChannelHandlerContextExt,
  EventExecutorGroupExt,
  GetPeers,
  GetPeersV2,
  KnownPeers,
  KnownPeersV2,
...
class PeerSynchronizer(peerDatabase: PeerDatabase, peerRequestInterval: FiniteDuration) extends ChannelInboundHandlerAdapter with ScorexLogging {
  ...
  def requestPeers(ctx: ChannelHandlerContext): Unit = if (ctx.channel.isActive) {
    val isPeerV2Supported = ctx.channel().hasAttr(Attributes.PeersV2Attribute)
    if (isPeerV2Supported) {
  ...
  override def channelRead(ctx: ChannelHandlerContext, msg: AnyRef): Unit = {
    ...
    msg match {
      ...
      case KnownPeers(peers) =>
        val added    = peerDatabase.addCandidates(peers)
        val notAdded = peers.filter(added.contains)
