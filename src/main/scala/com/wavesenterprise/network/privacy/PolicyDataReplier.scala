...
class EnablePolicyDataReplier(
  ...
  private def sendStrictResponse(channel: Channel, policyId: ByteStr, dataHash: PolicyDataHash): Task[Unit] = {
    (for {
      ...
      response   <- Task.fromEither(encryptStrictResponse(peers, channel, policyId, dataHash, loadedData.data, loadedData.metaData))
        ...
        .flatMap { peerConnection =>
          ...
          if (activeStreamRepliesCount >= maxPeersFullness) {
            ...
            (for {
              ...
              responseStream <- buildEncryptedResponseStream(peerConnection, loadStreamData)
            } yield {
