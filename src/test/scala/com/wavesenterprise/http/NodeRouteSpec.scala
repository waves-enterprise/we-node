...
import com.wavesenterprise.settings.{ConsensusSettings, FeeSettings, PlainGenesisSettings, WESettings}
...
class NodeRouteSpec extends RouteSpec("/node") with ApiSettingsHelper with PathMockFactory {
  ...
  private val testBlockchainSettings =
  ...
  private val weSettings = ConfigSource
    .fromConfig(config)
  ...
  val route: Route =
    new NodeApiRoute(weSettings, blockchain, new TestTime, ownerAccount, healthChecker).route
  ...
  routePath("/config") - {
    ...
    "check for PoA settings" in {
      ...
      val route: Route =
        new NodeApiRoute(settings, blockchain, new TestTime, ownerAccount, healthChecker).route
      Get(routePath("/config")) ~> route ~> check {
        response.status shouldBe StatusCodes.OK
        val parsedResponse = responseAs[NodeConfigResponse]
    ...
    "returns correct error" - {
      ...
      "on disabled service" - {
        ...
        val disabledExternalServicesRoute: Route =
          new NodeApiRoute(weSettings, blockchain, time, ownerAccount, disabledExternalServicesHealthChecker).route
