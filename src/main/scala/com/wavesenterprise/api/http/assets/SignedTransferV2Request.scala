...
case class SignedTransferV2Request(senderPublicKey: String,
  ...
  def toTx: Either[ValidationError, TransferTransactionV2] =
    for {
      ...
      _           <- TransferTransactionV2.ensureSupportedVersion(version)
      t           <- TransferTransactionV2.create(_sender, _assetId, _feeAssetId, timestamp, amount, fee, _recipient, _attachment.arr, _proofs)
    } yield t
}
