...
/**
  ...
  */
case class ExchangeTransactionDiff(blockchain: Blockchain, feeSettings: FeeSettings, height: Int) {

  def apply(tx: ExchangeTransaction): Either[ValidationError, Diff] = {
    ...
    val smartTradesEnabled = blockchain.activatedFeatures.contains(BlockchainFeature.SmartAccountTrading.id)
    val smartAssetsEnabled = blockchain.activatedFeatures.contains(BlockchainFeature.SmartAssets.id)
    for {
      ...
      def westPortfolio(amt: Long) = Portfolio(amt, LeaseBalance.empty, Map.empty)

      val feeDiff = Monoid.combineAll(
        Seq(
          Map(matcher -> westPortfolio(t.buyMatcherFee + t.sellMatcherFee - t.fee)),
          Map(buyer   -> westPortfolio(-t.buyMatcherFee)),
          Map(seller  -> westPortfolio(-t.sellMatcherFee))
        ))
