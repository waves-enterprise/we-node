...
import com.wavesenterprise.api.http.privacy.SignedCreatePolicyRequestV2
...
class CreatePolicyTransactionV2Specification extends AnyFunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {
  ...
  it("deserialize from json works") {
    ...
    val signedCreatePolicyRequest = SignedCreatePolicyRequestV2(
    ...
    val deserializedFromJson = tx.json().as[SignedCreatePolicyRequestV2]
  ...
  ignore("wrong proofs") {
    forAll(createPolicyTransactionV2GenWithRecipients()) { createPolicyTxWithRecipients =>
