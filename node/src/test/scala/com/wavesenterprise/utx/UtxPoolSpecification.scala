...
class UtxPoolSpecification
  ...
  private def createUtxPool(bcu: BlockchainUpdater with NG, time: Time, utxSettings: UtxSettings = defaultUtxSettings): UtxPool = {
    new UtxPoolImpl(
      time,
      bcu,
  ...
  private val withValidPayments = (for {
    ...
    val utxPool = createUtxPool(bcu, time)
    txs.foreach(utxPool.putIfNew(_, None))
  ...
  private def withScriptedAccount(scEnabled: Boolean): Gen[(PrivateKeyAccount, Long, UtxPool, Long)] =
    for {
      (sender, senderBalance, bcu) <- stateGen
  ...
  "UTX Pool" - {
    ...
    "smart accounts" - {
      "any transaction from scripted account is not allowed if smartAccounts disabled in utx pool" - {
        def enoughFeeTxWithScriptedAccount: Gen[(UtxPool, TransferTransaction)] =
          for {
