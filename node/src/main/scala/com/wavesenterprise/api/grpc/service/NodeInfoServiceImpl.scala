...
import com.wavesenterprise.settings.{AuthorizationSettings, CryptoSettings, WESettings}
...
class NodeInfoServiceImpl(val time: Time,
                          nodeSetting: WESettings,
                          cryptoSettings: CryptoSettings,
                          blockchain: Blockchain,
                          ownerPublicAccount: PublicKeyAccount)(implicit val s: Scheduler)
  ...
  override def nodeConfig(request: Empty, metadata: Metadata): Future[NodeConfigResponse] =
    withAuthTask(metadata) {
      Task {
        NodeConfigRawResponse.build(nodeSetting, cryptoSettings, blockchain).toProto
      }
    }.runToFuture
