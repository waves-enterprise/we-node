...
import com.wavesenterprise.state.diffs.{ENOUGH_AMT, assertDiffAndState}
...
class ContextFunctionsTest extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  def compactDataTransactionGen(sender: PrivateKeyAccount) =
    for {
      ...
      tx   <- dataTransactionV1GenP(sender, List(long, bool, bin, str))
    } yield tx
  ...
  val preconditionsAndPayments = for {
    ...
    transfer        <- transferGeneratorPV2(ts, master, recipient.toAddress, 100000000L)
    ...
    setScriptTransaction: SetScriptTransaction = SetScriptTransactionV1
      .selfSigned(CHAIN_ID, recipient, Some(typedScript), "script".getBytes(Charsets.UTF_8), Array.empty[Byte], 100000000L, ts)
      .explicitGet()
  ...
  property("base64 amplification") {
    val script =
      ...
      """.stripMargin
    runScript(script) shouldBe Left(s"base64Encode input exceeds ${WavesGlobal.MaxBase64Bytes}")
  }
}
