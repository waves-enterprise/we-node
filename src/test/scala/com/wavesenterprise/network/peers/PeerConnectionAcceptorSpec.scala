...
class PeerConnectionAcceptorSpec extends FreeSpec with Matchers with MockFactory with ScalaCheckPropertyChecks with TransactionGen {
  ...
  private def newConnection(connectionAcceptor: PeerConnectionAcceptor, blockchain: Blockchain, isValidator: Boolean = false): PeerConnection = {
    ...
    val handshake  = SignedHandshakeV2.createAndSign(applicationInstanceInfo, sessionKey, ownerKey)
