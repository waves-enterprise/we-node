package com.wavesenterprise.state.diffs.smart
...
import com.wavesenterprise.transaction.{DataTransactionV1, Transaction}
...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransactionV1, t: TransferTransaction): String =
  ...
  def scriptWithPureFunctions(tx: DataTransactionV1, t: TransferTransaction): String =
    s"""
       | # Pure context
  ...
  def scriptWithWavesFunctions(tx: DataTransactionV1, t: TransferTransaction): String =
    s""" # Waves context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       |   case _: DataTransaction => addressFromString("${tx.sender.address}") == Address(base58'${tx.sender.toAddress.bytes.base58}')
