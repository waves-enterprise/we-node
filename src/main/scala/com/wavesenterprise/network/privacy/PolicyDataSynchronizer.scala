...
object EnablePolicyDataSynchronizer extends ScorexLogging {
  ...
  object StreamResponse {
          ...
          case (accState, chunk) =>
            Task
              .defer {
                accState match {
                  case Empty =>
                    processFirstChunk(policyId, dataHash, chunk, peer, startPromise)
                  case HeaderAccumulation(out, written) =>
                    accumulateHeader(policyId, dataHash, chunk, written, out, peer, startPromise)
    ...
    private def processFirstChunk(
        policyId: ByteStr,
        dataHash: PolicyDataHash,
        chunk: Array[Byte],
        connection: PeerConnection,
        startPromise: Promise[StartDataLoadingEvent],
    )(implicit scheduler: Scheduler): Task[StreamProcessingAccumulator] =
      Task.defer {
        import crypto.context.algorithms.WrappedStructureLength
    ...
    private def accumulateHeader(
        ...
        connection: PeerConnection,
        startPromise: Promise[StartDataLoadingEvent],
    ): Task[StreamProcessingAccumulator] = Task.defer {
    ...
    private def accumulateHeaderWithEncryption(policyId: ByteStr,
                                               ...
                                               written: Int,
                                               startPromise: Promise[StartDataLoadingEvent]): Task[StreamProcessingAccumulator] = {
      import com.wavesenterprise.crypto.context.algorithms.WrappedStructureLength
