...
@Sharable
class NodeAttributesSender(ownerKey: PrivateKeyAccount, settings: NetworkSettings) extends ChannelInboundHandlerAdapter with ScorexLogging {
    ...
    msg match {
      case _: SignedHandshake =>
        ...
        if (channel.isOpen && channel.hasAttr(NodeAttributesKey)) {
          val nodeAttributes = NodeAttributes(settings.mode, p2pTlsEnabled = false, ownerKey)
