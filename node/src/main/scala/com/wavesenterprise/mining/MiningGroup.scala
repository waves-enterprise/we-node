...
case class SimpleTxSetup(tx: Transaction, maybeCertChain: Option[CertChain]) extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = SimpleGroupKey
}

case class AtomicSimpleSetup(tx: AtomicTransaction, innerSetups: List[SimpleTxSetup], maybeCertChain: Option[CertChain])
    extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = AtomicGroupKey
}

case class AtomicComplexSetup(tx: AtomicTransaction, innerSetupTasks: List[Task[TransactionConfirmationSetup]], maybeCertChain: Option[CertChain])
    extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = AtomicGroupKey
}
