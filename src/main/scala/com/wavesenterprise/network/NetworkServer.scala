...
import com.wavesenterprise.network.NS.{CloseCallback, NoActionCloseCallback}
import com.wavesenterprise.network.NetworkServer.ShutdownTimeout
...
trait NS {
  ...
  def incomingConnections: Observable[PeerConnection]

  def connect(remoteAddress: InetSocketAddress, closeCallback: CloseCallback): Future[PeerConnection]

  def messages: IncomingMessages
...
class NetworkServer private (
  ...
  private[this] val nodeAttributesSender = new NodeAttributesSender(ownerKey, networkSettings)
  ...
  override def start(): Future[InetSocketAddress] = {
  ...
  override def connect(remoteAddress: InetSocketAddress, closeCallback: CloseCallback = NoActionCloseCallback): Future[PeerConnection] = {
    val connectPromise = Promise[PeerConnection]
    ...
    channelFuture
      .addListener { f: io.netty.util.concurrent.Future[_] =>
        if (f.isSuccess) {
          log.trace(s"${id(channelFuture.channel())} Connection established")
        } else {
...
object NetworkServer extends ScorexLogging {
  ...
  def apply(networkSettings: NetworkSettings,
            applicationInfo: ApplicationInfo,
            lastBlockInfos: Observable[LastBlockInfo],
            blockchain: BlockchainUpdater with NG,
            historyReplier: HistoryReplier,
