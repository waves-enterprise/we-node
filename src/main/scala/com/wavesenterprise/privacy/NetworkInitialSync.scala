...
class NetworkInitialSync(
  ...
  private val networkClient = new InitialSyncNetworkClient(ownerKey, settings.network)
    ...
    responses match {
      ...
      case _ =>
        ...
        val (failures, successes) = responses.map {
          case SuccessPeerIdentityResponse(pk) => Right(PublicKeyAccount(pk))
        ...
        if (failures.size == responses.size) {
          ...
          val result = successes
            .filter(pka => blockchain.participantPubKey(pka.toAddress).isEmpty)
            .map(pka => (pka.toAddress, pka))
...
class InitialSyncNetworkClient(
    ownerKey: PrivateKeyAccount,
      ...
      .handler(
        new ChannelInitializer[SocketChannel] {
          override def initChannel(ch: SocketChannel): Unit = {
            ch.pipeline()
              .addLast(new PeerIdentityRequestEncoder,
