...
package object http extends ApiMarshallers {
  ...
  private def parseAndSignTransaction(typeId: Byte,
                                      version: Byte,
                                      txJson: JsObject,
                                      wallet: Wallet,
                                      time: Time): Either[ValidationError, ProvenTransaction] = {
    TransactionParsers.by(typeId, version) match {
      case None => Left(GenericError(s"Bad transaction type ($typeId) and version ($version)"))
      case Some(txParser) =>
        txParser match {
  ...
  def signTransaction(jsv: JsObject, wallet: Wallet, time: Time): Either[ApiError, ProvenTransaction] = {
    getFilledTransaction(jsv) match {
      case Left(parseErrors) => Left(WrongJson(None, parseErrors.errors))
      case Right(txJsonWithMeta) =>
        val typeId  = txJsonWithMeta.typeId
        ...
        parseAndSignTransaction(typeId, version, txJson, wallet, time).leftMap(ApiError.fromValidationError)
    }
  }
