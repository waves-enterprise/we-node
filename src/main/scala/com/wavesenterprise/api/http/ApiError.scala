...
sealed trait ApiError extends IntEnumEntry {
  def id: Int = value
  val message: String
  val code: StatusCode
...
object ApiError extends IntEnum[ApiError] {
  ...
  /**
    * PKI-related errors
    */
  case object SignatureFormatError extends ApiError {
    override val value: Int                    = 623
    override val code: StatusCodes.ClientError = StatusCodes.BadRequest
    override val message: String               = "Signature format not supported"
  }

  case class ExtendedKeyUsageError(missingEKUs: List[String]) extends ApiError {
    override val value: Int                    = 624
    override val code: StatusCodes.ClientError = StatusCodes.BadRequest
    override val message: String =
      s"""Error applying Extended Key Usage, signer's certificate is missing requested OIDs: ${missingEKUs.mkString("; ")}"""
  }

  case class CertificateSearchError(detailedMessage: String) extends ApiError {
    override val value: Int       = 625
    override val code: StatusCode = StatusCodes.InternalServerError
    override val message: String  = s"Error in CAdES: $detailedMessage"
  }

  case class CRLSearchError(detailedMessage: String) extends ApiError {
    override val value: Int       = 626
    override val code: StatusCode = StatusCodes.InternalServerError
    override val message: String  = s"Error in CAdES: $detailedMessage"
  }

  case object CryptoNotAvailableError extends ApiError {
    override val value: Int       = 627
    override val code: StatusCode = StatusCodes.InternalServerError
    override val message: String  = "Crypto-specific method called expecting unavailable algorithms."
  }

  case object SignatureCreationError extends ApiError {
    override val value: Int       = 628
    override val code: StatusCode = StatusCodes.BadRequest
    override val message: String =
      "CAdES signature creation failed. Possibly incorrect signature type has been requested or the signature is corrupted"
  }
