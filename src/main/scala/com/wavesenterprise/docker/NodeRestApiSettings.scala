...
object NodeRestApiSettings {
  ...
  def apply(url: HttpUrl): NodeRestApiSettings = new NodeRestApiSettings(url.host, url.port, url.toString)
  ...
  private def resolveNodeRestApiSettings(
    ...
    if (dockerEngineSettings.useNodeDockerHost) {
      ...
      NodeRestApiSettings(host, restAPIPort, s"http://$host:$restAPIPort").asRight
    }
  }
