...
class PoAConsensusSpec extends FreeSpec with Matchers with ScalaCheckPropertyChecks with TransactionGen with NoShrink {
  ...
  "Select miner without skipping" - {
    "when previous miner is in the beginning of the list" in {
      ...
      val poa = new PoAConsensus(blockchain, blockchainSettings, poaSettings)
...
object PoAConsensusSpec extends MockFactory {
  ...
  def mockMyBlockchain(minerAccount: PrivateKeyAccount, addressToPermissions: Map[Address, PermissionOp]): NG =
  ...
  def mockMyBlockchain(minerAccount: PrivateKeyAccount,
                       ...
                       banHistory: Map[Address, MinerBanHistory]): NG = {
    val blockchain: NG = mock[NG]
    ...
    (blockchain.miners _).expects().returning(miners)
    ...
    if (banHistory.nonEmpty) {
      banHistory.foreach {
        case (address, minerBanHistory) =>
          (blockchain.minerBanHistory(_: Address)).expects(address).returning(minerBanHistory).noMoreThanOnce()
      }
    } else {
