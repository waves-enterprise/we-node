...
class CommonValidationTimeTest extends PropSpec with ScalaCheckPropertyChecks with Matchers with TransactionGen with NoShrink with WithState {
    ...
    forAll(for {
      ...
      case (prevBlockTs, blockTs, height, transfer1) =>
        withStateAndHistory(TestFunctionalitySettings.Enabled) { blockchain: Blockchain =>
          ...
          )(blockchain, transfer1) should produce("too old")
        }
    }
  }
      ...
      case (prevBlockTs, blockTs, height, transfer1) =>
        withStateAndHistory(TestBlockchainSettings.Default.custom.functionality) { blockchain: Blockchain =>
          TransactionDiffer(TestBlockchainSettings.Default,
                            ...
                            blockTs,
                            height,
                            MaxTimePrevBlockOverTransactionDiff)(blockchain, transfer1) should produce("far future")
        }
    }
  }
}
