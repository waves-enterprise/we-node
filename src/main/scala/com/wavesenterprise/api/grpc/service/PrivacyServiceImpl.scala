...
import com.wavesenterprise.api.http.{ApiError, PolicyItem, PrivacyDataInfo, SendDataRequest}
...
class PrivacyServiceImpl(privacyService: PrivacyApiService,
  ...
  private def saveStreamToStorage(dataLoadingStart: StartDataLoadingEvent, dataStream: Observable[Byte]): Task[PolicyDataHashTransaction] =
    EitherT
      .fromEither[Task](privacyService.validateLargePolicyItem(dataLoadingStart.request.toPolicyItem))
      .leftMap(ApiError.fromValidationError)
      .flatMapF { setup =>
        Task.fromFuture {
          privacyService.sendLargeData(dataLoadingStart.request.toPolicyItem, setup, dataStream, broadcast = dataLoadingStart.broadcast)
        }
      }
      .valueOrF { apiError =>
...
object PrivacyServiceImpl extends ScorexLogging {
  ...
  object StreamProcessingAccumulator {
    ...
    case class MetaDataAccumulated(request: SendDataRequest, broadcast: Boolean) extends StreamProcessingAccumulator
  ...
  case class StartDataLoadingEvent(request: SendDataRequest, broadcast: Boolean, dataStream: Observable[Array[Byte]])
