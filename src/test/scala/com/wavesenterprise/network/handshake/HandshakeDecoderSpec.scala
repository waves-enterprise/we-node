...
class HandshakeDecoderSpec extends FreeSpec with Matchers with MockFactory with ScalaCheckPropertyChecks with TransactionGen with NoShrink {
  ...
  "should read a signed handshake and remove itself from the pipeline" in {
    ...
    val sessionKey    = crypto.generatePublicKey
    val origHandshake = SignedHandshakeV2.createAndSign(applicationInstanceInfo, sessionKey, account)
  ...
  "should read a signed handshake with declaredAddress and remove itself from the pipeline" in {
    ...
    val account                       = accountGen.sample.get
    val sessionKey                    = crypto.generatePublicKey
    val origHandshake                 = SignedHandshakeV2.createAndSign(customApplicationInstanceInfo, sessionKey, account)
  ...
  "should comparing SignedHandshakeV2 in a right way" in {
    val address    = addressGen.sample.get
    val sessionKey = crypto.generatePublicKey
    val bytesGen   = byteArrayGen(crypto.SignatureLength)
    val signedPairGen = for {
      bytes1 <- bytesGen
      bytes2 <- bytesGen
    } yield (Signature(bytes1), Signature(bytes2))

    forAll(signedPairGen) {
      case (s1, s2) =>
        val sh1 = SignedHandshakeV2(applicationInstanceInfo, address, sessionKey, s1)
        val sh2 = SignedHandshakeV2(applicationInstanceInfo, address, sessionKey, s2)
        val sh3 = SignedHandshakeV2(applicationInstanceInfo, address, sessionKey, s1)
