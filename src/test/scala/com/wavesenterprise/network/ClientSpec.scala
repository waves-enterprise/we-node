...
class ClientSpec extends FreeSpec with Matchers with MockFactory with TransactionGen {
  ...
  "should add a server's channel to all channels after the handshake only" in {
    ...
    val handshake                   = SignedHandshakeV2.createAndSign(clientApplicationInstanceInfo, sessionPubKey, ownerKey)
    ...
    val channel = createEmbeddedChannel(activePeerConnections, clientApplicationInstanceInfo, blockchain, otherOwnerKey)
  ...
  "shouldn't connect to self" in {
    ...
    val handshake = SignedHandshakeV2.createAndSign(clientApplicationInstanceInfo, crypto.generatePublicKey, sameOwnerKey)
  ...
  "should not accept handshake with other consensus type" in {
    ...
    val buff      = Unpooled.buffer
    val handshake = SignedHandshakeV2.createAndSign(clientApplicationInstanceInfo, crypto.generatePublicKey, ownerKey)
    handshake.encode(buff)
    buff.writeCharSequence("foo", StandardCharsets.UTF_8)
  ...
  "should accept signedHandshake with accepted address if privacy settings expect signedHandshake" in {
    ...
    val signedHandshake = SignedHandshakeV2.createAndSign(clientApplicationInstanceInfo, sessionKey, ownerAccount)
  ...
  "should not accept signedHandshake with not accepted address if privacy settings expect signedHandshake" in {
    ...
    val sessionKey         = crypto.generatePublicKey
    val signedHandshake    = SignedHandshakeV2.createAndSign(clientApplicationInstanceInfo, sessionKey, senderAccount)
  ...
  "should not accept signedHandshake with invalid signature if privacy settings expect signedHandshake" in {
    ...
    val signedHandshake    = SignedHandshakeV2(clientApplicationInstanceInfo, address, sessionKey, signature)
