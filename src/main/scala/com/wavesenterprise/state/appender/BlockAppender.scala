...
import com.wavesenterprise.transaction.{BlockchainUpdater, _}
...
class BlockAppender(
  ...
  private def applyExtension(channel: Channel, extensionBlocks: Seq[Block]): Task[Unit] = Task.defer {
    log.debug(s"Attempting to append extension '${formatBlocks(extensionBlocks)}' from '${id(channel)}'")
    extensionBlocks.foreach(BlockStats.received(_, BlockStats.Source.Extension, channel))

    (for {
      validBlocks                <- EitherT(signatureValidator.validateOrdered(extensionBlocks))
      blockIdToPreValidatedTxIds <- EitherT.right[GenericError](preValidateBlocksTxs(validBlocks).asyncBoundary(scheduler))
      processResult              <- EitherT.fromEither[Task](processValidExtensionBlocks(validBlocks, blockIdToPreValidatedTxIds))
    } yield {
      log.info(
        s"Successfully processed extension '${formatBlocks(extensionBlocks)}' from '${id(channel)}'. " +
      ...
      val message = s"Error appending extension '${formatBlocks(extensionBlocks)}' from '${id(channel)}': '$error'"
      log.warn(message)
  ...
  private def processValidExtensionBlocks(blocks: List[Block],
                                          blockIdToPreValidatedTxIds: Map[ByteStr, Set[ByteStr]]): Either[ValidationError, ExtensionProcessResult] = {
    ...
    newBlocks.headOption.map(_.reference) match {
      ...
      case Some(extensionReference) =>
        ...
        for {
          ...
          _ <- appendExtensionBlocks(extensionReference, newBlocks, blockIdToPreValidatedTxIds).leftFlatMap { err =>
  ...
  private def appendExtensionBlocks(extensionReference: ByteStr,
                                    blocks: List[Block],
                                    blockIdToPreValidatedTxIds: Map[ByteStr, Set[ByteStr]]): Either[ValidationError, Unit] = {
    blocks.zipWithIndex
      .traverse {
        case (block, i) =>
          val alreadyVerifiedTxIds = blockIdToPreValidatedTxIds.getOrElse(block.uniqueId, Set.empty)
          baseAppender
            .appendBlock(block, Hard, alreadyVerifiedTxIds = alreadyVerifiedTxIds)
            .map { maybeNewHeight =>
      ...
      case (ch, block) =>
        Task.defer {
          if (blockchainUpdater.isRecentlyApplied(block.uniqueId)) {
            Task.pure {
              log.debug(s"Block '$block' from '${id(ch)}' already in the state")
            }
          } else {
