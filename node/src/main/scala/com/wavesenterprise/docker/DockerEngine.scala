...
trait DockerEngine extends ExecuteCommandInDocker {
  ...
  def executeBashCommandInContainer(
      containerId: String,
      command: String,
      privileged: Boolean = true
  ): Either[ContractExecutionException, (Int, String)]

  def createAndStartContainer(
      contract: ContractInfo,
      metrics: ContractExecutionMetrics,
      envParams: List[String]
  ): Either[ContractExecutionException, String]

  def executeRunScript(
      containerId: String,
...
object DockerEngine {
  ...
  private def checkRegistryAuthOnStartup(
      dockerEngineSettings: DockerEngineSettings,
      dockerClient: DockerClient
...
private class DockerEngineImpl(val docker: DockerClient, dockerEngineSettings: DockerEngineSettings) extends DockerEngine {
  ...
  private def deployContainer(
    ...
    for {
      ...
      containerCreation = docker
        .createContainerCmd(imageInfo.getId)
        .withHostConfig(createContainerHostConfig())
        .withEnv(envParams: _*)
