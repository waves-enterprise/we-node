...
trait Blockchain extends ContractBlockchain with PrivacyBlockchain {
  def height: Int
  def score: BigInt
  ...
  def append(diff: Diff, carryFee: Long, block: Block, consensusPostActionDiff: ConsensusPostActionDiff = ConsensusPostActionDiff.empty): Unit
