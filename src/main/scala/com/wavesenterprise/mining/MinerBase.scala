...
import com.wavesenterprise.mining.Miner.{MicroBlockMiningResult, _}
...
trait MinerBase extends Miner with Instrumented with ScorexLogging {
  ...
  protected def initNewConfirmation(keyBlockId: BlockId): MinerTransactionsConfirmatory = {
    ...
    new MinerTransactionsConfirmatory(
      ...
      utx,
      settings.miner.pullingBufferSize,
      settings.miner.utxCheckDelay,
      ownerKey
    )(scheduler)
  }
          ...
          event match {
            case CheckTimeoutEvent =>
              if (accTxs.nonEmpty && time.correctedTime() - lastMicroTime >= microBlockIntervalMillis) {
                ...
                generateAndAppendMicroBlock(confirmatory, account, accLiquidBlock, accTxs.reverse, restTotalConstraint, parallelLiquidGeneration)
  ...
  protected def generateAndAppendMicroBlock(confirmatory: MinerTransactionsConfirmatory,
    ...
    buildAndAppendMicroBlock(account, liquidBlock, time.correctedTime(), Left(transactions), parallelLiquidGeneration)
      .flatMap {
  ...
  protected def buildAndAppendMicroBlock(account: PrivateKeyAccount,
    ...
    (for {
      microAndLiquid <- EitherT {
        measureTask(
          microBlockFullBuildTimeStats,
          buildMicroAndLiquidPair(account, liquidBlock, currentTs, txsOrVotes, parallelLiquidGeneration)
        )
      }
      (microBlock, liquidBlock) = microAndLiquid
      _ <- EitherT(microBlockAppender.appendMicroBlock(microBlock, isOwn = true))
    } yield {
  ...
  private def buildMicroBlock(account: PrivateKeyAccount,
                              ...
                              currentTs: Long,
                              txsOrVotes: Either[Seq[Transaction], Seq[Vote]],
                              totalSignature: ByteStr = ByteStr.empty): Either[ValidationError, MicroBlock] = {
    measureSuccessful(
  ...
  private def buildMicroAndLiquidPair(account: PrivateKeyAccount,
                                      liquidBlock: Block,
                                      currentTs: Long,
                                      txsOrVotes: Either[Seq[Transaction], Seq[Vote]],
                                      parallelLiquidGeneration: Boolean): Task[Either[ValidationError, (MicroBlock, Block)]] = {
    if (parallelLiquidGeneration) {
      (
        Task(buildLiquidBlock(account, liquidBlock, txsOrVotes)),
        Task(buildMicroBlock(account, liquidBlock, currentTs, txsOrVotes))
      ).parMapN {
        case (newLiquidResult, newMicroWithoutSignatureResult) =>
          for {
            newLiquid                <- newLiquidResult
            newMicroWithoutSignature <- newMicroWithoutSignatureResult
          } yield {
            newMicroWithoutSignature.withTotalSignature(newLiquid.uniqueId) -> newLiquid
          }
      }
    } else {
      (for {
        newLiquid <- EitherT(Task(buildLiquidBlock(account, liquidBlock, txsOrVotes)))
        newMicro  <- EitherT(Task(buildMicroBlock(account, liquidBlock, currentTs, txsOrVotes, newLiquid.uniqueId)))
      } yield newMicro -> newLiquid).value
    }
  }
