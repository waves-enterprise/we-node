...
case class ExecutableTxSetup(tx: ExecutableTransaction, executor: ContractExecutor, info: ContractInfo, parallelism: Int)
    extends TransactionConfirmationSetup {
  val groupKey: ContractExecutionGroupKey = ContractExecutionGroupKey(parallelism)
}

case class SimpleTxSetup(tx: Transaction) extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = SimpleGroupKey
}

case class AtomicSimpleSetup(tx: AtomicTransaction, innerSetups: List[SimpleTxSetup]) extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = AtomicGroupKey
}

case class AtomicComplexSetup(tx: AtomicTransaction, innerSetupTasks: List[Task[TransactionConfirmationSetup]]) extends TransactionConfirmationSetup {
  override def groupKey: GroupKey = AtomicGroupKey
}
