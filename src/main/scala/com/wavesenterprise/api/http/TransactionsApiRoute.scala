...
import com.wavesenterprise.api
...
class TransactionsApiRoute(val settings: ApiSettings,
                           ...
                           val scheduler: SchedulerService)(implicit ex: ExecutionContext)
    extends ApiRoute
  ...
  def sign: Route = (pathPrefix("sign") & post & userAuth) {
    pathEndOrSingleSlash {
      withExecutionContext(scheduler) {
        json[JsObject] { jsv =>
          log.trace(s"receive json to sign: '$jsv'")
          signTransaction(jsv, toValidateFee = true)
        }
      }
    }
  }
  ...
  /**
    ...
    **/
  def signAndBroadcast: Route = (pathPrefix("signAndBroadcast") & post & nonWatcherFilter & userAuth) {
    pathEndOrSingleSlash {
      withExecutionContext(scheduler) {
        json[JsObject] { jsv =>
          (for {
            txJson      <- EitherT.fromEither[Task](signTransaction(jsv))
            tx          <- EitherT.fromEither[Task](TransactionFactory.fromSignedRequest(txJson)).leftMap(ApiError.fromValidationError)
            validatedTx <- validateAndBroadcastTransaction(tx)
  ...
  private def signTransaction(jsv: JsObject, toValidateFee: Boolean = false): Either[ApiError, JsObject] = {
    for {
      signedTx <- api.http.signTransaction(jsv, wallet, time)
      _ <- if (toValidateFee) {
        feeCalculator.validateTxFee(blockchain.height, signedTx).left.map(ApiError.fromValidationError)
      } else {
        Right(())
      }
    } yield signedTx.json()
  ...
  private def txToCompactJson(address: Address, tx: Transaction): JsObject = {
    ...
    tx match {
      case mtt: MassTransferTransaction if mtt.sender.toAddress != address =>
        val addresses = (blockchain.aliasesOfAddress(address) :+ address).toSet
        mtt.json() ++ Json.obj(
